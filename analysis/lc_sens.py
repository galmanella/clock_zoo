...
def cycle_profiles(model, params, m=64, n_steps=1024):
    """(profiles[n_states, m], T) for a parameter dict, or (None, nan) if the orbit fails."""
    import jax
    from engine.orbit import OrbitSolver
    m2 = type(model)()
    m2.set_parameters(params)
    s = OrbitSolver(m2, n_steps=n_steps)
    P = m2.jax_params()
    y0, T, res = jax.jit(s.solve)(P, s.guess(P))
    if not np.isfinite(float(res)) or float(res) > 1e-6:
        return None, np.nan                        # did not converge: report, do not guess
    C = np.asarray(s.cycle(P, y0, T, m))           # (m, n_states)
    return C.T, float(T)


def run_param(model, name, base, obs_idx, C0, T0, scale, factors=FACTORS, m=64):
    """Per-factor LC displacement for one parameter."""
    ...
    for k, fac in enumerate(factors):
        pd = dict(base)
        pd[name] = base[name] * float(fac)
        try:
            C, T = cycle_profiles(model, pd, m=m)
        except Exception as e:                     # never let one parameter kill the sweep
            print(f"    [{name} x{fac}] orbit FAILED {type(e).__name__}: {e}", flush=True)
            continue
        if C is None:
            continue
        d = (C - C0) / scale[:, None]
...
def run(model_name, shard=None, nshards=None, tag=None, m=64, factors=FACTORS):
    ...
    C0, T0 = cycle_profiles(model, base, m=m)
    if C0 is None:
        raise RuntimeError(f"{model_name}: the BASE orbit did not converge")
...
def report(blob, top=15):
    ...
    if dead:
        print(f"  {dead} parameter(s) had no converged orbit at any factor", flush=True)
...
def merge(model_name, tag=None):
    ...
    for k in ('shape_all', 'shape_obs', 'dT'):
