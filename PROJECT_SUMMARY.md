...
All regenerated by `python -m analysis.figures --model M --target T` -- a pure read of the
saved npz, so none of them costs compute to rebuild.

| figure | what it answers |
|---|---|
| `almeida_scrit_pulse.png` | winding vs dose per target; where each one changes type, and where the numerics stop being trustworthy (shaded) |
| `almeida_surfaces_pulse.png` | the base PTC surfaces: singularity, S_crit, and the twist curve |
| `almeida_tornado_BMAL1_pulse.png` | LC and PTC sensitivity per parameter on a shared ordering, plus their ratio |
| `almeida_lc_examples.png` | what an LC sensitivity number looks like as an actual cycle change |
| `almeida_ptc_examples_BMAL1_pulse.png` | the same for the PTC, with the Delta new-phase panel |
| `almeida_directions_BMAL1_pulse.png` | the combinatorial decoupling: spectrum, parameters vs directions, loadings |
| `almeida_coupling_BMAL1.png` | the per-parameter LC-vs-PTC scatter |

**The black line on a PTC surface is the twist curve** -- the stable fixed point (the
attracting entrainment phase) as a function of dose. Where it shears across the dose axis is
where the isochrons are twisted. The dashed white line is S_crit and the white dot is the
phase singularity.
